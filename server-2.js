...
/* ============================================================
   JAILBREAK FLAG REPORTING
   Buat tabel di Supabase SQL Editor:

   CREATE TABLE jailbreak_flags (
...
// POST /api/flag — laporan jailbreak dari frontend (publik, tidak butuh auth)
app.post('/api/flag', async (req, res) => {
  try {
    const { user_email, user_name, severity, preview, time } = req.body;
    if (!preview) return res.status(400).json({ error: 'Preview diperlukan.' });
    const { error } = await supabaseAdmin.from('jailbreak_flags').insert({
    ...
    // Jangan return error ke client — jangan beri info ke attacker
    res.json({ success: true });
  }
});

// GET /api/admin/flags — ambil semua laporan jailbreak (admin only)
app.get('/api/admin/flags', requireAdmin, async (req, res) => {
  try {
    const { data, error } = await supabaseAdmin
      .from('jailbreak_flags')
      .select('*')
...
// PUT /api/admin/flags/:id/review — tandai sudah ditinjau
app.put('/api/admin/flags/:id/review', requireAdmin, async (req, res) => {
  try {
